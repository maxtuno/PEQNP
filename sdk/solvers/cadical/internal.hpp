...
struct Internal {
  ...
  bool force_dec;
  bool save_trail;
  size_t old_trail_qhead;
  vector<int> old_trail;
  vector<Clause*> old_trail_reasons;
  vector<int> on_old_trail;
  ...
  OrderSchedule order;          // order based resolution priority queue
  ...
  int gap;
  uint64_t global_stable_allUip_attempt;
  uint64_t global_stable_allUip_success;
  int stable_allUip_attempt;
  int stable_allUip_success;
#ifndef QUIET
  ...
  inline void clear_old_trail() {
    old_trail.clear();
    old_trail_reasons.clear();
    old_trail_qhead = -1;
  }
  ...
  void analyze ();
  void stable_allUip(int lbd);
  bool check_allUIP_resovlable(int p, Clause* confl);
  void clean_up_seen3(vector<int>& to_be_cleaned);
  ...
  //update stable_allUip control variable "gap"
  void update_stable_allUip_gap();
...
inline bool res_smaller::operator () (unsigned a, unsigned b){
  assert (1 <= a);
  assert (1 <= b);
  Var v_a = internal->vtab[a];
  Var v_b = internal->vtab[b];

  if(v_a.level < v_b.level){
    return true;
  }else if(v_a.level > v_b.level){
    return false;
  }else{
    return (v_a.trail <= v_b.trail);
  }
