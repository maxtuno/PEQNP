...
void Stats::print (Internal * internal) {
  ...
  if (all || stats.conflicts) {
  PRT ("learnt clauses:         %15" PRId64 "   %10.2f %%  per conflict", stats.learned.clauses, percent (stats.learned.clauses, stats.conflicts));
  ...
  PRT ("learnt literals:       %15" PRId64 , stats.learned.literals - stats.minimized - stats.alluip_minimized);
  PRT ("minimized:       %15" PRId64 "   %10.2f %%  minimized", stats.minimized, percent (stats.minimized, stats.learned.literals));
  PRT ("all_uip minimized:       %15" PRId64 "   %10.2f %%  minimized by all-uip", stats.alluip_minimized, percent (stats.alluip_minimized+stats.minimized, stats.learned.literals));
  PRT ("all_uip attempt rate:    %10.2f %% ", percent (internal->global_stable_allUip_attempt, stats.learned.clauses));
  PRT ("all_uip success rate:    %10.2f %% ", percent (internal->global_stable_allUip_success, internal->global_stable_allUip_attempt));
