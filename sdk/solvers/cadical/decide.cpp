...
int Internal::decide () {
  ...
  if ((size_t) level < assumptions.size ()) {
    ...
    int decision;
    if (force_dec && val(old_trail[old_trail_qhead]) == 0 && old_trail_reasons[old_trail_qhead] == 0) {
      //if (val(old_trail[old_trail_qhead]) > 0) printf("HERE!\n");
      //printf("FORCED %d\n", old_trail[old_trail_qhead]);
      decision = old_trail[old_trail_qhead];
    }
    else {
      int idx = next_decision_variable ();
      const bool target = opts.stabilizephase && stable;
