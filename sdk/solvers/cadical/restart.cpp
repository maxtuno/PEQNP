...
void Internal::restart () {
  ...
  //stable-allUIP
  update_stable_allUip_gap();

  STOP (restart);
}

void Internal::update_stable_allUip_gap() {

  bool i_uip_failed = ((stable_allUip_attempt > 0) && 
            ((double)stable_allUip_success / (double)stable_allUip_attempt) < 0.8);
  if (i_uip_failed){
            gap += 1;
  }else{
      int tentative_gap = gap -1;
      gap = (tentative_gap <= 0) ? 0 : tentative_gap;
  } 
  
  global_stable_allUip_success += stable_allUip_success;;
  global_stable_allUip_attempt += stable_allUip_attempt;
