...
void Internal::clean_up_seen3(vector<int>& to_be_cleaned){
  for (int lit: to_be_cleaned){
    flags (lit).seen3 = 0;
  }
...
bool Internal::check_allUIP_resovlable(int p, Clause* confl){
  for (const auto & q : *confl){
    if (q == p){
      continue;
    }else{
      int q_level = var(q).level;
      if (!flags (q).seen3 && q_level > 0){
        Level & l = control[q_level];
        if(l.seen.count == 0){ 
          return false;
        }
    }
...
void Internal::stable_allUip (int lbd){
  int c_size = clause.size();
  //check if should performe stable-allUIP
  if (c_size <= (gap + lbd)){
    return;
  }
  stable_allUip_attempt++;
  vector<int> to_be_cleaned;
  //vector<CRef> clause_to_be_bumped;

  vector<int> new_out_learnt;
  ...
  for(int lit : clause){
      flags (lit).seen3 = true;
      to_be_cleaned.push_back(lit);
      Var & v = var(lit);
      order.push_back(vidx(lit)); 
  }

    int new_literal = 0;
    int grace_token = c_size - lbd;
    //int top_index =0;
    //int current_decision_level = top_i[top_index];
    int i = 0;

      while (!order.empty()){
          Var & v = var(order.pop_front());
          int p = trail[v.trail];
          int p_level = v.level;
          Clause* confl = v.reason;
          flags (p).seen3= 0;
          //assert(level(var(p)) == current_decision_level);
          //add the literal into the learnt clause if it is the last literal in a decision level
          if (order.empty() || p_level != var(order.front()).level){
              assert( order.empty() || p_level > var(order.front()).level);
              new_out_learnt.push_back(-p);
              flags (-p).seen3 = true;
              to_be_cleaned.push_back(-p);
              continue;
          }
        
          //check if we should perform resolution
          bool should_res = check_allUIP_resovlable(p, confl);
          if (!should_res){
            new_out_learnt.push_back(-p);
            flags (-p).seen3 = 1;
            to_be_cleaned.push_back(-p);
            ...
            if (grace_token == 0){
              ...
              for (const auto & q : *confl){
                if (q == p){
                  ...
                  if (!flags (q).seen3 &&  q_level > 0){
                      flags (q).seen3 = 1;
                      to_be_cleaned.push_back(q);
                      order.push_back(vidx(q));
                  }
              }
          }
      }
    ...
    //to_be_cleaned.assign(new_out_learnt.begin(), new_out_learnt.end());
    if (opts.minimize){
       //minimize_clause(new_out_learnt, new_out_learnt, 0);
    }
    
    int old_size = clause.size();
    int size_delta = clause.size() - new_out_learnt.size();
    if (size_delta > 0){
        if (opts.stable_allUIP_active && opts.score){
          double old_min = 1e150;
          double new_min = 1e150;


          for (int lit: clause){
            double old_score = score(vidx(lit));
            old_min = old_score < old_min?  old_score : old_min;
          }

          for (int lit: new_out_learnt){
            if (!flags (lit).seen){
              double new_score = score(vidx(lit));
              new_min = new_score < new_min?  new_score : new_min;
            }
          }

          if (  old_min > (new_min + scinc)){
            clean_up_seen3(to_be_cleaned);
            return;
          }  
        }

       stable_allUip_success++;
       //stats.alluip_minimized += size_delta;
       clause.clear();
       clause.assign(new_out_learnt.begin(), new_out_learnt.end()); 
         if (opts.stable_allUIP_exclusive && opts.score){
          for (int lit : clause){
            if (!flags (lit).seen){
                bump_score(lit);
            } 
        ...
        stats.alluip_minimized += (old_size - clause.size());
     }
...
void Internal::analyze () {
  ...
  if (opts.stable_allUIP){
    START (stable_allUip);
    stable_allUip(glue+1);
    size = (int) clause.size ();
    STOP (stable_allUip);
  }
