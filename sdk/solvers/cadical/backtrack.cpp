...
void Internal::backtrack (int new_level) {
  ...
  if (save_trail && !opts.chrono) {
    old_trail.resize((int) (end_of_trail - i));
    old_trail_reasons.resize((int) (end_of_trail - i));
  }
  ...
  while (i < end_of_trail) {
    ...
    if (v.level > new_level) {
      ...
      unassigned++;
      if (save_trail && !opts.chrono) {
        old_trail[end_of_trail - i] = lit;
        old_trail_reasons[end_of_trail - i] = v.reason;
      }
    } else {
  ...
  old_trail_qhead = old_trail.size() - 1;
