...
:
  ...
  opts (this),
  gap (0),
  stable_allUip_attempt(0),
  stable_allUip_success(0),
#ifndef QUIET
  profiles (this),
...
int Internal::cdcl_loop_with_inprocessing () {
  ...
  while (!res) {
    save_trail = true;
         if (unsat) res = 20;
    else if (!propagate ()) { force_dec = false; analyze (); }      // propagate and analyze
    ...
    else if (restarting ()) { save_trail = false; clear_old_trail(); restart (); }      // restart by backtracking
    else if (rephasing ()) { save_trail = false; clear_old_trail(); rephase (); }       // reset variable phases
    else if (reducing ()) { save_trail = false; clear_old_trail(); reduce (); }         // collect useless clauses
    else if (probing ()) { save_trail = false; clear_old_trail(); probe (); }           // failed literal probing
    else if (subsuming ()) { save_trail = false; clear_old_trail(); subsume (); }       // subsumption algorithm
    else if (eliminating ()) { save_trail = false; clear_old_trail(); elim (); }        // variable elimination
    else if (compacting ()) { save_trail = false; clear_old_trail(); compact (); }      // collect variables
    else if (conditioning ()) { save_trail = false; clear_old_trail(); condition (); }  // globally blocked clauses
