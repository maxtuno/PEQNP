...
class pixie {
  ...
  public:
    ...
    void set_integer_condition(const std::vector<bool> &integers) { ints.assign(integers.begin(), integers.end()); }
    ...
    double optimize() {
        result result(vc);
        auto aux = optimize(result);
        opt = aux.z;
        values = aux.get_variables();
        return opt;
    }

    double get_variable(int i) { return values[i]; }
